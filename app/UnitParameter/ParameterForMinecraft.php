...
class ParameterForMinecraft extends ParameterForWebsocket
{
    ...
    public function getCommandDataForLightSwordItemUsed(float $p_x, float $p_y, float $p_z, float $p_yrot, int $p_variant): array
    {
        ...
        {
            $cmd = "setblock {$w_x} {$w_y} {$w_z} customize:light_shield [\"customize:block_mode\":1]";
            $cmd_datas[] = $this->getCommandData($cmd, null);
        }
        // シールドモード展開
        else
        {
            $cmd = "fill {$w_x} {$w_y} {$w_z} {$w_x} {$to_y} {$w_z} customize:light_shield [\"customize:block_mode\":2]";
            $cmd_datas[] = $this->getCommandData($cmd, null);
    ...
    /**
     * アイアンファング用コマンドデータを取得（右クリック時）
     * 
     * @return array 送信データ
     */
    public function getCommandDataForIronFangItemUsed(): array
    {
        $cmd_datas = [];

        $cmd = "event entity @s customize:iron_fang";
        $cmd_datas[] = $this->getCommandData($cmd, 'iron-fang-event');
